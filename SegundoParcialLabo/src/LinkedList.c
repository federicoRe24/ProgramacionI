...
/** \brief acumula elementos devueltos por la funcion criterio recibida como parametro
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return int Retorna  el valor acumulado ó (-1) Error: si el puntero a la listas es NULL
 */
 ...
 * \return int Retorna  el valor acumulado ó (-1) Error: si el puntero a la listas es NULL
 */
int ll_reduceIntById(LinkedList* this, int (*pFunc)(void*, int), int id)
{
	int acumulador = 0;
	int len = ll_len(this);
	void* pElement;
    if (this != NULL && pFunc != NULL)
    {
    	...
    	{
    		pElement = ll_get(this, i);
    		acumulador = acumulador + pFunc(pElement, id);
    	}
    }
    return acumulador;
}
...
/** \brief filtra los elementos de una lista utilizando la funcion criterio recibida como parametro
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return int Retorna  (-1) Error: si el puntero a la listas es NULL
                                ( 0) Si ok
